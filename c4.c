...
Collision (ball * ballz)
{
  ...
  while (head)
      ...
      if (IsHit (ballz, head))
	{
	  ...
	  if (ballz->lasthit == head  && head->lasthit == ballz)
		return (0);

          // Impact= balla-ballb         vector subtaction
	  impact_x = head->delta_x - ballz->delta_x + 1;
	  impact_y = head->delta_y - ballz->delta_y + 1;

          // impulse = Normalize(p1,p2) 
	  ...
	  ballz->delta_y = hdy;
          ...
          // offset things a little bit to reduce multiple collisions
	  head->x += 2;
	  head->y += 2;
...
LoadBallz ()
{
  ...
  if (current = NewBall ("ball.bmp", 100, 400, 5, -15, 2))
    {
      if (head)
	{
	  current->next = head;
  ...
  if (current = NewBall ("BigBlueBall.bmp", 400, 100, -30, 4, 1))
    {
      if (head)
	{
	  current->next = head;
  ...
  if (current = NewBall ("ball.bmp", 300, 300, -10, 2, 2))
    {
      if (head)
	{
	  current->next = head;
  ...
  if (current = NewBall ("BigBlueBall.bmp", 64, 200, 15, 16, 1))
    {
      if (head)
	{
	  current->next = head;
